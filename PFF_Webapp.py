...
def get_data():
    df = pd.read_csv('https://raw.githubusercontent.com/cdav15/Portfolio/main/PFF_Pass.csv')
...
tabs = ['Home Screen','Passing','Receiving','Rushing']
...
if selected_tab == 'Home Screen':
    ...
    st.write('Fill in background info here')
    
    
    st.write('insert example graph image')
    st.write('insert key and example graph explanation')
    
    st.write('insert a pic and description of how to use the webapp')
    ...
    if not player_select:
        ...
        fig.text(
            0.515, 0.942,
            "Passing Percentile Ranks (min. 100 attempts)",
            size=15,
            ha="center", color="#000000"
        )
        ...
        CREDIT_2 = "Software developed by Chandler Davis"
        CREDIT_3 = "@CADgolfer /x and @chandler___davis /instagram"
        ...
        download_filename = st.text_input('Choose File Name:', value='QB_Graph_1')
...
#####################
        ...
        fig_text(
            0.515, 0.99, f"<{p2}> vs <{p3}>", size=17, fig=fig2,
        ...
        fig2.text(
            0.515, 0.942,
            "Passing Percentile Rankings Comparison",
            size=15,
            ha="center", color="#000000"
        )
        ...
        st.write('#### Download Graph as Image Below:')
        download_filename1 = st.text_input('Choose File Name:', value='QB_Comparison')
        
        img_bytes2 = io.BytesIO()
        plt.savefig(img_bytes2, format='png')
        
        file_name01 = download_filename1 + '.jpg'
        
        st.download_button(
            label="Download Graph as PNG",
            data=img_bytes2,
            file_name=file_name01,
            mime='image/png',
        )
        st.write('')  
...
# elif selected_tab == 'Receiving':
    st.title('PFF Receiving Stats and Grades Percentile Rankings')
    
    dfwr = receiving_data()
    
    
    st.write("I've created this web app to help fans, analysts, and coaches better understand the strengths and weaknesses NFL pass catchers.")
    st.write("The data is filtered to players with 25 or more targets during the 2023 Regular Season.")
    st.write("                  ")
    st.write('Original Data Provided Below')
    st.dataframe(dfwr)
    ...
    st.write("## WR Strengths and Weaknesses")
    wr_select = st.selectbox("Choose a player:", list(dfwr.index))
    
    if not wr_select:
        st.error("Please Select a Player.")
    else:
        dfwr2 = dfwr.drop(['player_id', 'position', 'bats', 'team_name', 'player_game_count', 
                         'declined_penalties', 'def_gen_pressures', 'first_downs', 'franchise_id',
                        'hit_as_threw','penalties', 'spikes',
                        'thrown_aways'], axis=1)
